...
const ResetPassword: React.FC<FormState> = () => {
  ...
  const onSubmit = async (event: { preventDefault: () => void }) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}user/reset-password?t=${t}`,
        { password }
      );
  ...
  return (
    <div className={`m-auto `}>
      ...
      <div
        className={` flex min-h-screen items-center justify-center flex-col m-auto pt-10`}
      >
        {isSuccessful ? (
          <div
            className={`space-y-4 flex h-full flex-col items-center justify-center m-auto ${styles.animation}`}
          >
            ...
            <Link
              href="/auth/login"
              target="_self"
              className={`text-white bg-purpleBase hover:bg-purple7 p-4 rounded-xl mt-10`}
            >
              Proceed to Log in
            </Link>
          </div>
          ...
          <div>
            <div
              className={` flex items-center flex-col my-2 space-y-2 mx-auto text-center px-2  w-[400px]`}
            >
              ...
              <form action="" onSubmit={onSubmit} className={`space-y-5 `}>
                <div className={`flex flex-col`}>
                  <div className="flex justify-between items-end my-2">
                    ...
                    <p
                      className={`text-red10 text-[10px] w-[260px] h-[45px] text-justify`}
                    >
                      {formData.error1}
                    </p>
                  </div>
                ...
                <div className={`flex flex-col`}>
                  <div className="flex justify-between items-end my-2">
                    ...
                    <p
                      className={`text-red10 text-[10px] w-[240px] h-[20px] text-justify`}
                    >
                      {formData.error2}
                    </p>
                  </div>
            ...
            <div className="text-red5 mt-2 flex justify-center items-center">
              {errorState}
            </div>
          </div>
        )}
