...
interface OpenListingProps {
  isLoading: any;
  currentCategory: any;
  activeListings: any;
}

const OpenListingCategory = ({
  isLoading,
  currentCategory,
  activeListings,
}: OpenListingProps) => {
  ...
  const [currentActiveListings, setCurrentActiveListings] = useState([]);
  const [nthPageforActive, setNthPageForActive] = useState(0);

  useEffect(() => {
    const indexOfLastListing = currentPage * listingsPerPage;
    const indexOfFirstListing = indexOfLastListing - listingsPerPage;

    // Update currentActiveListings based on pagination
    const updatedActiveListings = activeListings.slice(
      indexOfFirstListing,
      indexOfLastListing
    );
    setCurrentActiveListings(updatedActiveListings);

    // Update nthPageforActive based on the length of activeListings
    const updatedNthPageForActive = Math.ceil(
      activeListings.length / listingsPerPage
    );
    setNthPageForActive(updatedNthPageForActive);
  }, [currentPage, activeListings]);
          ...
          {currentCategory === "Open" && activeListings.length === 0 ? (
            ...
            <div className="flex flex-col justify-center items-center  w-[800px]  ">
              <div className=" grid grid-cols-3 gap-10 h-[600px] ">
                {currentActiveListings.map((listing: any) => (
                  <Link
...
export default OpenListingCategory;
