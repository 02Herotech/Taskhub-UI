...
const AllTaskCategory = ({
  ...
  const [currentAllTasks, setCurrentAllTasks] = useState([]);
  ...
  useEffect(() => {
    ...
    // Update currentAllTasks based on pagination
    const updatedAllTasks = taskData.slice(indexOfFirstTask, indexOfLastTask);
    setCurrentAllTasks(updatedAllTasks);

    // Update nthPageforAll based on the length of taskData
    const updatedNthPageForAll = Math.ceil(taskData.length / tasksPerPage);
