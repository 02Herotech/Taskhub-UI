...
const OpenTaskCategory = ({
  ...
  useEffect(() => {
    ...
    const updatedActiveTasks = activeTasks.slice(
      indexOfFirstTask,
      indexOfLastTask
    );
    setCurrentActiveTasks(updatedActiveTasks);

    // Update nthPageforActive based on the length of activeTasks
    const updatedNthPageForActive = Math.ceil(
      activeTasks.length / tasksPerPage
    );
            ...
            <div className="flex flex-col ">
              <div className="h-[700px] ">
                <div className=" flex flex-col items-center bg-[#F8E9FE]/[0.5] px-5 ">
                  {currentActiveTasks.map((task: any) => (
                    <Link
...
export default OpenTaskCategory;
